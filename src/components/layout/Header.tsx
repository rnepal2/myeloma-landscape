  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#09272d]/95 text-white shadow-[0_8px_30px_rgba(8,38,44,0.12)] backdrop-blur-xl">
      <div className="mx-auto flex h-[72px] max-w-[1280px] items-center gap-7 px-5 sm:px-7">
        ...
        >
          <span className="grid size-12 place-items-center rounded-full border border-[#8fe3c5]/30 bg-[#8fe3c5]/10 text-[#8fe3c5]">
            <Radar size={28} />
          </span>
        ...
        <nav
          className={cx(
            "absolute left-0 right-0 top-[72px] flex-col border-b border-[#dbe5e1] bg-white p-3 shadow-xl md:static md:flex md:flex-row md:items-center md:gap-1 md:rounded-full md:border md:border-white/12 md:bg-white/[0.10] md:p-1 md:shadow-none",
        ...
        >
          {navigation.map((item) => (
            <NavLink
              className={({ isActive }) =>
                cx(
                  "rounded-full px-3.5 py-2.5 text-xs font-bold no-underline transition md:py-2",
                  isActive
